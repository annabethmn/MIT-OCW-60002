...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        if dirt_amount < 0:
            raise ValueError("Dirt amount must be greater than or equal to zero")
        self.width = width 
        self.height = height
        self.dirt_amount = dirt_amount
        self.tiles = {}
        for x in range (math.floor(width)):
            for y in range(math.floor(height)):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        tile_coord = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        current_dirt_amount = self.tiles[tile_coord]
        if capacity <= current_dirt_amount:
            new_dirt_amount = current_dirt_amount - capacity 
        else:
            new_dirt_amount = 0 
        self.tiles[tile_coord] = new_dirt_amount
    ...
    def is_position_in_room(self, pos):
        ...
        """
        tile_coord = (math.floor(pos.get_x()), math.floor(pos.get_y()))
        return tile_coord in self.tiles.keys()
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        tile_coord = (int(pos.get_x()), int(pos.get_y()))
        return tile_coord in self.furniture_tiles
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
                  robot_type):
    #faulty robot outputs lower than expected 
    """
    ...
    """
    total_time_steps = 0
    robot_room = EmptyRoom(width, height, dirt_amount) 
    for i in range(num_trials):
        #OPTIONAL: initialize animation of trial- uncomment following line
        #anim = ps3_visualize.RobotVisualization(num_robots, width, height, 0, delay=0.2)
        robot_room = EmptyRoom(width, height, dirt_amount) 
        robots = []
        for j in range(num_robots):
            new_robot = robot_type(robot_room, speed, capacity)
            robots.append(new_robot)
        tiles_cleaned = 0.0 #percentage of tiles in the room with no dirt
        while tiles_cleaned < min_coverage:
            #simulate robot position and cleaning at 1 time step
            for robot in robots:
                robot.update_position_and_clean() 
            #OPTIONAL: update animation- uncomment following line
            #anim.update(robot_room, robots)
            total_time_steps += 1
            #calculate percentage of tiles cleaned
            tiles_cleaned = (robot_room.get_num_cleaned_tiles())/(robot_room.get_num_tiles())
        #end animation
        #anim.done()
    return (total_time_steps/num_trials)
# print(run_simulation(1, 1.0, 1.0, 5.0, 5.0, 1, 1.0, 100, StandardRobot))
